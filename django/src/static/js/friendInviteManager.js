class FriendInviteManager {
	...
	showDialog() {
		...
		if (!dialog) {
			...
			// Ajouter des styles inline pour le fond et autres
			dialog.style.backgroundColor = '#0D0D0D'; // Fond de la modale
			dialog.style.color = '#FFFFFF'; // Texte blanc pour contraste
			...
			dialog.innerHTML = `
				<h3 style="margin-bottom: 15px; font-size: 18px; font-weight: bold; color: #FFFFFF;">${this.customTitle}</h3>
				<p style="margin-bottom: 10px; color: #FFFFFF;">Select friends to invite:</p>
				<div id="friendsList" style="max-height: 300px; overflow-y: auto; background-color: #0d0d0d; padding: 10px; border-radius: 4px;">
					<p style="color: #FFFFFF;">Loading friends list...</p>
				</div>
				<div style="margin-top: 20px; text-align: right;">
					<button id="cancelInviteBtn" style="margin-right: 10px; padding: 8px 12px; border: 1px solid #ccc; background: #0d0d0d; color:#ffffff; border-radius: 4px;">Cancel</button>
        ...
        .then(users => {
            ...
            friends.forEach(friend => {
                ...
                if (friend.profile_picture) {
                    profilePicHtml = `<img src="${friend.profile_picture}" alt="${friend.username}" class="profile-picture-invite">`;
                } else {
                    profilePicHtml = `<div class="profile-picture-invite bg-gray-300" style="width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background-color: #ccc;"><span>${friend.username.charAt(0).toUpperCase()}</span></div>`;
                }
